...
def generate_by_term(query):
    current_term = query["query"].get("term", "")
    new_queries = []
    for c in ascii_lowercase:
        new_query = deepcopy(query)
        new_query["query"]["term"] = f"{current_term}{c}"
        new_queries.append(new_query)
    return new_queries


def get_or_set(dict_, key, default_key):
    if key not in dict_:
        dict_[key] = default_key
        return default_key
    else:
        return dict_[key]
...
def get_stat_subdict(dict, stat_id):
    ...
    dict["query"]["stats"].append(
        {"type": "and", "filters": [{"id": stat_id, "value": {}, "disabled": False,}],}
    )
...
def generate_by_stat_range(
    ...
    return generate_by_range_from_subdict(query_copy, sub_dict, default_min_value, default_max_value, rounding_precision=rounding_precision)
...
def generate_by_range_from_subdict(query, sub_dict, default_min_value, default_max_value, rounding_precision=2):
...
def generate_by_roll_ranges(query, result):
    ...
    for mod_hash, range_ in hash_to_range.items():
        ...
        possible_splits = generate_by_stat_range(query, mod_hash, current_min, current_max)
        if possible_splits:

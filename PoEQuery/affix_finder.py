...
added_filters = {"implicit" : StatFilters(filters=[StatFilter(id="pseudo.pseudo_number_of_implicit_mods", min=1)]),
                #  "explicit" : StatFilters()}
                 "explicit" : StatFilters(filters=[StatFilter(id="pseudo.pseudo_number_of_affix_mods", min=1)]),
                 "fractured" : StatFilters()}


def simplify_stat_filter(stat_filters: Set[StatFilter], threshold=1):
    """
        ...
        for stat_filter_1, stat_filter_2 in combinations(stat_filters,2):
            if stat_filter_1.id == stat_filter_2.id:
                ...
                if overlap_check_1 or overlap_check_2:
                    ...
                    other_stat_filters = {stat_filter for stat_filter in stat_filters if stat_filter != stat_filter_1 and stat_filter != stat_filter_2}
                    combined_stat_filter = StatFilter(id=stat_filter_1.id, min=min(stat_filter_1.min,stat_filter_2.min), max=max(stat_filter_1.max,stat_filter_2.max))
...
def find_affixes(official_query, affix_type="explicit", exclude_affixes=[], use_added_filter_for_speed=True):
    ...
    if not results and use_added_filter_for_speed:
        logging.warn("For unique items you might need to set use_added_filter_for_speed=False")
    ...
    while results and added_mod:
        ...
        if(found_mods & new_found_mods):
            logging.warning(f"found a mod that should be excluded: {found_mods & new_found_mods} (for jewelry, try setting quality_max=0)")
        added_mod = len(new_found_mods.difference(found_mods)) > 0
        found_mods = found_mods | new_found_mods
        stat_filter.filters = simplify_stat_filter([stat_filter for mod in found_mods for stat_filter in mod.to_query_stat_filters()])

...
def compare_dicts(dict_1, dict_2):
    ...
    if isinstance(dict_1, dict):
        assert set(dict_1.keys()) == set(dict_2.keys()), f"{set(dict_1.keys()).symmetric_difference(set(dict_2.keys()))}"
        ...
        assert len(dict_1) == len(dict_2), f"different lengths of lists, {len(dict_1)} {len(dict_2)}"
...
def sort_prices(df):
    ...
    """
    sorted_price_columns = sorted([c for c in df.columns if c.startswith("price_")], key= lambda x: int(x[6:]))
    other_columns = [c for c in df.columns if c not in sorted_price_columns]
    df = df[other_columns + sorted_price_columns]

    for idx in tqdm(range(len(df))):
        df.loc[idx,sorted_price_columns] = df.loc[idx,sorted_price_columns].sort_values().values

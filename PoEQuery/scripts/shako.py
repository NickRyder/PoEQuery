...
attr_mod = Mod({'name': '', 'tier': '', 'magnitudes': [{'hash': 'explicit.stat_1379411836', 'min': 25, 'max': 30}]})
mods = find_affixes(OfficialApiQuery(name="Forbidden Shako", type="Great Crown", identified=True), affix_type="explicit", exclude_affixes= [attr_mod],use_added_filter_for_speed=False)
...
for mod in tqdm(mods):
    ...
    fetch_ids, total, query = search_query(OfficialApiQuery(name="Forbidden Shako", type="Great Crown",  identified=True, corrupted=False, enchanted=False, indexed="2week", stat_filters=[mod_stat_filters]).to_json())
    print(mod, total)

...
from PoEQuery.official_api import fetch_query, fetch_results, recurse_fetch_query_with_query_divider
from PoEQuery.query_generator import generate_by_ilvl, generate_by_price, generate_by_roll_ranges
...
query = {"query":{"status":{"option":"any"},"stats":[{"type":"and","filters":[{"id":"enchant.stat_3948993189","value":{"option":24},"disabled":False},{"id":"enchant.stat_3086156145","value":{"min":5,"max":5},"disabled":False}],"disabled":False}]},"sort":{"price":"asc"}}

fetch_ids, unsplit_queries = recurse_fetch_query_with_query_divider(
    query, [generate_by_ilvl, generate_by_price]
    )

for query in unsplit_queries:
    print(query)

results = fetch_results(fetch_ids)


with open("./data/aura_jewels_cold_8.json", "w") as f: 
    json.dump(results, f)

...
life_mod = Mod({'name': '', 'tier': '', 'magnitudes': [{'hash': 'explicit.stat_983749596', 'min': 4, 'max': 6}]})
mana_mod = Mod({'name': '', 'tier': '', 'magnitudes': [{'hash': 'explicit.stat_2748665614', 'min': 4, 'max': 6}]})
es_mod = Mod({'name': '', 'tier': '', 'magnitudes': [{'hash': 'explicit.stat_2482852589', 'min': 4, 'max': 6}]})

mods = find_affixes(OfficialApiQuery(name="Watcher's Eye", type="Prismatic Jewel", identified=True), affix_type="explicit", exclude_affixes= [life_mod, mana_mod, es_mod],use_added_filter_for_speed=False)
...
for mod in tqdm(mods):
    ...
    fetch_ids, total, query = search_query(OfficialApiQuery(name="Watcher's Eye", type="Prismatic Jewel", identified=True, corrupted=False, indexed="2week", stat_filters=[mod_stat_filters]).to_json())
    print(total)

...
mods = find_affixes(OfficialApiQuery(category="armour.chest", corrupted=False, rarity="nonunique", links_w=6, identified=True), affix_type="explicit")
...
for mod in tqdm(mods):
    ...
    fetch_ids, total, query = search_query(OfficialApiQuery(category="armour.chest", corrupted=False, rarity="nonunique", links_w=6, identified=True, mirrored=False, indexed="2week", stat_filters=[mod_stat_filters]).to_json())
    print(total)

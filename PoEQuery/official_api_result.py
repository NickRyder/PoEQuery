...
class Price:
    ...
    def __init__(self, json):
        self.type_, self.amount, self.currency = (
            json["type"],
            json["amount"],
            json["currency"],
        )
...
class Mod:
    ...
    def to_query_stat_filters(self):
        ...
        for magnitude in self.magnitudes:
            average_found_affix_hashes[magnitude.hash_].append(
                (magnitude.min_, magnitude.max_)
            )
        ...
        for hash_, min_max_tuples in average_found_affix_hashes.items():
            ...
            min_average, max_average = min(min_average, max_average), max(
    ...
    def __init__(self, json):
        ...
        try:
            self.implicits = tuple(
                [
                    Mod(imp)
                    for imp in json["item"]["extended"]["mods"]["implicit"]
                    if any(imp.values())
                ]
            )
        except KeyError:
        ...
        try:
            self.explicits = tuple(
                [
                    Mod(imp)
                    for imp in json["item"]["extended"]["mods"]["explicit"]
                    if any(imp.values())
                ]
            )
        except KeyError:
        ...
        try:
            self.enchants = tuple(
                [
                    Mod(imp)
                    for imp in json["item"]["extended"]["mods"]["enchant"]
                    if any(imp.values())
                ]
            )
        except KeyError:
        ...
        try:
            self.fractureds = tuple(
                [
                    Mod(imp)
                    for imp in json["item"]["extended"]["mods"]["fractured"]
                    if any(imp.values())
                ]
            )
        except KeyError:
        ...
        try:
            self.crafteds = tuple(
                [
                    Mod(imp)
                    for imp in json["item"]["extended"]["mods"]["crafted"]
                    if any(imp.values())
                ]
            )
        except KeyError:
